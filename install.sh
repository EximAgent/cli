#!/usr/bin/env sh
# eximagent installer — pins v0.1.732.
set -eu
...
tag="v0.1.732"
echo "[eximagent] downloading v0.1.732 for ${os}-${arch}..." >&2
if ! curl -fsSL "$url" -o "$tmp"; then
  ...
  tag=""
  echo "[eximagent] v0.1.732 is no longer published; falling back to the latest release" >&2
  curl -fsSL "$url" -o "$tmp"
fi
