# eximagent installer (Windows) — pins v0.1.731.
...
$url = "https://github.com/EximAgent/cli/releases/download/v0.1.731/$asset"
$tag = 'v0.1.731'
Write-Host "[eximagent] downloading v0.1.731 for windows-amd64..."
try { Invoke-WebRequest -Uri $url -OutFile $tmp -UseBasicParsing } catch {
  ...
  $tag = ''
  Write-Host "[eximagent] v0.1.731 is no longer published; falling back to the latest release"
