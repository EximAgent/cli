...
[n] gives a flag's description or enum; required markers stay inline.

[1] `Collection name or id`
[2] `Company UUID for the lead.`
[3] `From email (operator-owned, verified)`
[4] `HS code (4/6/8/10 digit prefix)`
[5] `HS code for the traded market (required) — ANY granularity (2/4/6-digit); a shorter code is a broader family (e.g. 0901 = all coffee). Do NOT pad to 6 digits.`
[6] `JSON array of steps {waitDays,subject,body,variants?:[{subject,body}]}; variants A/B auto-converge to the best by click-rate`
[7] `Outreach brief — what to mention, tone, ask`
[8] `Profile name`
[9] `Receiving country — accepts country name, ISO2, ISO3 (e.g. "Germany", "DE", "DEU")`
[10] `Reply review UUID`
[11] `Sender display name`
[12] `Sending country — accepts country name, ISO2, ISO3 (e.g. "Viet Nam", "VN", "VNM")`
[13] `Sequence UUID`
[14] `Signature appended via {signature}`
[15] `Signature appended via {signature} in templates`
[16] `Subject line (may use {{variables}})`
[17] `Target collection ID`
[18] `Template body. Supports {{variable}} placeholders + [FILE_ID:storageId:filename] attachment markers.`
[19] `Template name`
[20] `Template name or id`
[21] `Template name to delete. Discover via `template list`.`
[22] `UUID of the target collection`
[23] `Workflow JSON.`
[24] `destination country (ISO 2) — the market whose movement to read`
[25] `enum: export|import`
[26] `enum: high|low|medium`
[27] `enum: high|medium|low`
[28] `enum: month|quarter`
[29] `share token`
[30] `share token to revoke`
[31] `shipment id`
